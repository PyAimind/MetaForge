...
with tempfile.TemporaryDirectory() as tmp:
    ...
    assert os.path.commonpath([os.path.abspath(config.OUTPUT_DIR), os.path.abspath(nested_path)]) == os.path.abspath(config.OUTPUT_DIR)
    ...
    for _ in range(3):
        ...
        files_set.add(os.path.basename(got.payload["filepath"]))
    assert files_set == {"multi_0.py", "multi_1.py", "multi_2.py"}
    ...
    assert isinstance(res_bad_phase.phase, int)
